...
const AppImage = memo(function AppImage({
    ...
    const sanitizeSrc = useCallback((rawSrc: string): string => {
        if (!rawSrc) return fallbackSrc;
        // Fix URLs that are missing the protocol (e.g. "www.pichat.us/image.jpg")
        if (typeof rawSrc === 'string' && !rawSrc.startsWith('/') && !rawSrc.startsWith('http') && !rawSrc.startsWith('data:')) {
            return `https://${rawSrc}`;
        }
        return rawSrc;
    }, [fallbackSrc]);

    const [imageSrc, setImageSrc] = useState(() => sanitizeSrc(src));
